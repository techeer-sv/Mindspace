import {
    Controller, Get, Query, Res, HttpStatus, Post, Body, Headers, Param, Put, Delete,
} from '@nestjs/common';
import { BoardService } from './board.service';
import {ApiTags, ApiOperation, ApiQuery, ApiParam, ApiHeader, ApiResponse} from '@nestjs/swagger';
import { Response } from 'express';
import { CreateBoardDto } from './dto/create-board.dto';
import {Board} from "./entities/board.entity";
import {BoardDto} from "./dto/node-boards.dto";
...
export class BoardController {
    ...
    @Get('/all')
    async getAllBoardsByNodeId(@Query('node_id') nodeId: number): Promise<BoardNodeResponseDto[]> {
    ...
    ): Promise<BoardResponseDto> { // <-- 변경된 반환 타입
    ...
    @ApiResponse({ status: 200, description: '게시글 조회 성공', type: SpecificBoardNodeDto })
    ...
    @ApiResponse({ status: 200, description: '게시글 조회 성공', type: BoardDetailDto })
    @ApiResponse({ status: 404, description: '게시글을 찾을 수 없습니다.' })
