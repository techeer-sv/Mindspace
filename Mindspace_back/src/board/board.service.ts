...
export class BoardService {
    ...
    async getAllBoardsByNodeId(nodeId: number): Promise<BoardNodeResponseDto[]> {
        const boards = await this.boardRepository.find({ where: {nodeId: nodeId} });
        return boards.map(board => BoardMapper.BoardNodeResponseDto(board));
        //여기서 dto수정해주면 responsebody에 보이는 게 달라짐
    }

    async createBoard(nodeId: number, userId: string, createBoardDto: CreateBoardDto): Promise<BoardResponseDto> {
        const convertedUserId = Number(userId);
        const user = await this.userService.findUserById(convertedUserId);
        const userNickname = user.nickname;

        const board = this.boardMapper.dtoToEntity(createBoardDto, nodeId, convertedUserId, userNickname);
        const savedBoard = await this.boardRepository.save(board);
        return BoardMapper.boardToResponseDto(savedBoard);
    }

    async getBoardById(id: number): Promise<BoardResponseDto> {
        const boardEntity: Board = await this.boardRepository.findOne({ where: { id: id } });
        return BoardMapper.boardToResponseDto(boardEntity);
    }

    async updateBoard(nodeId: number, userId: string, updateBoardDto: UpdateBoardDto): Promise<BoardResponseDto> {
        const convertedUserId = Number(userId);  // userId를 number로 변환

        const board = await this.boardRepository.findOne({ where: { nodeId: nodeId, userId: convertedUserId } }); // 변환된 userId를 사용
        ...
        // 사용자 권한 검사
        if (board.userId !== convertedUserId) {  // 변환된 userId를 사용
    ...
    async deleteOwnBoard(nodeId: number, userId: string): Promise<void> {
        ...
        const board = await this.boardRepository.findOne({ where: { id: nodeId, userId: convertedUserId } });
    ...
    async getBoardByNodeIdAndUserId(nodeId: number, userId: number): Promise<SpecificBoardNodeDto> {
        const board = await this.boardRepository.findOne({ where: { nodeId: nodeId, userId: userId } });
        if (!board) {
